import React, { useCallback } from "react";
import { getOltInfo, getPonPorts, getOnus } from "../services/api";
import { useOltData } from "../services/useOltData";
import "./Dashboard.css";

function StatCard({ label, value, sub, color }) {
  return (
    <div className="stat-card" style={{ borderTop: `3px solid ${color}` }}>
      <div className="stat-value" style={{ color }}>{value}</div>
      <div className="stat-label">{label}</div>
      {sub && <div className="stat-sub">{sub}</div>}
    </div>
  );
}

function Dashboard() {
  const fetchInfo    = useCallback(getOltInfo,  []);
  const fetchPon     = useCallback(getPonPorts, []);
  const fetchOnus    = useCallback(getOnus,     []);

  const { data: oltInfo,   loading: l1, error: e1, lastUpdated } = useOltData(fetchInfo);
  const { data: ponPorts,  loading: l2, error: e2 }              = useOltData(fetchPon);
  const { data: onus,      loading: l3, error: e3 }              = useOltData(fetchOnus);

  const loading = l1 || l2 || l3;
  const error   = e1 || e2 || e3;

  const onlinePon  = (ponPorts || []).filter((p) => p.status === "up").length;
  const offlineOnu = (onus     || []).filter((o) => o.status === "offline").length;

  if (loading && !oltInfo) {
    return <div className="page-content"><div className="loading-state">Loading dashboard…</div></div>;
  }

  if (error && !oltInfo) {
    return <div className="page-content"><div className="error-state">⚠ {error}</div></div>;
  }

  const info = oltInfo || {};
  const ports = ponPorts || [];

  return (
    <div className="page-content">
      <div className="page-header">
        <h1 className="page-title">Dashboard</h1>
        <span className="page-meta">
          Real-time OLT Overview
          {lastUpdated && <span className="last-updated"> · Updated {lastUpdated.toLocaleTimeString()}</span>}
        </span>
      </div>

      <div className="olt-info-card">
        <div className="olt-info-grid">
          <div className="olt-field">
            <span className="olt-field-label">Device Name</span>
            <span className="olt-field-value">{info.name}</span>
          </div>
          <div className="olt-field">
            <span className="olt-field-label">IP Address</span>
            <span className="olt-field-value">{info.ip}</span>
          </div>
          <div className="olt-field">
            <span className="olt-field-label">Location</span>
            <span className="olt-field-value">{info.location}</span>
          </div>
          <div className="olt-field">
            <span className="olt-field-label">Uptime</span>
            <span className="olt-field-value">{info.uptime}</span>
          </div>
          <div className="olt-field">
            <span className="olt-field-label">Model</span>
            <span className="olt-field-value">{info.model}</span>
          </div>
          <div className="olt-field">
            <span className="olt-field-label">Firmware</span>
            <span className="olt-field-value">{info.firmware}</span>
          </div>
        </div>
      </div>

      <div className="stats-grid">
        <StatCard label="Total PON Ports" value={info.totalPon}    sub={`${onlinePon} Online`}         color="#58a6ff" />
        <StatCard label="Total GE Ports"  value={info.totalGe}     sub="Active"                        color="#3fb950" />
        <StatCard label="Total ONU"       value={info.totalOnu}    sub={`${info.onlineOnu} Online`}    color="#d2a8ff" />
        <StatCard label="ONU Offline"     value={offlineOnu}       sub="Need Attention"                color="#f85149" />
      </div>

      <div className="pon-overview">
        <h2 className="section-title">PON Port Overview</h2>
        <div className="pon-grid">
          {ports.map((port) => (
            <div key={port.id} className={`pon-box pon-${port.status}`}>
              <div className="pon-name">{port.name}</div>
              <div className="pon-count">{port.onuCount} ONU</div>
              <div className="pon-status">{port.status === "up" ? "UP" : "DOWN"}</div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
}

export default Dashboard;
